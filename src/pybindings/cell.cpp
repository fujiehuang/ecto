...
struct cellwrap : cell, bp::wrapper<cell> {
  ...
  struct YouveBeenServed {
    void operator()(const tendrils::value_type &t) { t.second->notify(); }
  };
...
const tendrils &inputs(cell &mod) { return mod.inputs; }
tendrils &outputs(cell &mod) { return mod.outputs; }
tendrils &params(cell &mod) { return mod.parameters; }

void wrapModule() {
  // use private names so that python people know these are internal
  bp::class_<cell, boost::shared_ptr<cell>, boost::noncopyable>("_cell_cpp",
                                                                bp::no_init)
      .def("type", &cell::type)
  ...
  bp::class_<cellwrap, boost::shared_ptr<cellwrap>, boost::noncopyable>(
      "_cell_base" /*bp::no_init*/)
      .def("_set_process_connected_inputs_only",
           &cell::set_process_connected_inputs_only)
      ...
      .def("activate", ((void (cell::*)()) & cell::activate))
      .def("deactivate", ((void (cell::*)()) & cell::deactivate))
      .def("process_with_only_these_inputs",
           (void (cell::*)()) & cell::process_with_only_these_inputs)
  ...
  bp::class_<TendrilSpecifications>("TendrilSpecifications",
                                    bp::init<bp::list>())
...
void inspect_impl(ecto::cell_ptr m, const boost::python::tuple &args,
  ...
  if (bp::len(args) > 1)
    throw std::runtime_error("Only one non-keyword argument allowed, this will "
  ...
  if (bp::len(args) == 0) {
    ...
    if (!e.check())
      throw std::runtime_error(
          "Non-keyword argument (instance name) not convertible to string.");
    m->name(e());
  }
  m->declare_params();
